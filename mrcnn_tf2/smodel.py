...
class MaskRCNN(KM.Model):
    ...
    def _get_inputs_shape(self):
        
        config = self.get_config()
        bsize = config.BATCH_SIZE
        inputs_shape =[(bsize,)+tuple(config.IMAGE_SHAPE),
                       (bsize,)+(config.IMAGE_META_SIZE,),
                       (bsize,)+(None,), 
                       (bsize,)+(None,4),
                       (bsize,)+(config.MINI_MASK_SHAPE[0],
    ...
    def call(self, inputs):
        ...
        # self.counter +=1
        # print('Counter:{}, inputshapes:{}, {}, {}, {}, {}, {}, {}'.
        #       format(self.counter, inputs[0].shape, inputs[1].shape,
    ...
    def detect(self, images, verbose=0):
        ...
        detections, mrcnn_class, mrcnn_bbox, mrcnn_mask, rpn_rois, rpn_class, rpn_bbox =\
            self.predict([molded_images, image_metas], verbose=0)
        # Process detections
        results = []
