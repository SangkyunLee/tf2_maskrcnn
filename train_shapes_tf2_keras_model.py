...
from mrcnn_tf2 import kmodel as modellib
...
@tf.function
def compute_loss(model,config, inputs):
...
@tf.function
def compute_apply_gradients(model, config, inputs, optimizer):
  with tf.GradientTape() as tape:
    loss = compute_loss(model, config, inputs)
...
output_types=((tf.float32, tf.float32, tf.int32, tf.float32, tf.bool, tf.int32, tf.float32),())    
...
for epoch in range(epochs):
    for step, inputs_ in enumerate(dataset):
        ...
        if step==0 and epoch ==0:            
            ...
            model.fpn.load_weights('./mask_rcnn_coco.h5',by_name = True)
        
        loss = compute_apply_gradients(model.mrcnn, model.config, inputs, optimizer)
        print('epoch:{}. step {},  loss={}'.format(epoch, step,loss))
        losses.append(loss)



# # model.mrcnn.save_weights('test_all.h5')
# model.fpn.save_weights('test-fpn.h5')
# model.fpn.load_weights('test-fpn.h5')

# model.rpn.save_weights('test-rpn.h5')
# model.rpn.load_weights('test-rpn.h5')

# model.fpn_clf.save_weights('test-fpn_clf.h5')
# model.fpn_clf.load_weights('test-fpn_clf.h5')
